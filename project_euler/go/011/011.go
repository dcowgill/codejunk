...
func solve() int {
	...
	search := func(xb, xe, xi, yb, ye, yi int) int {
		m := 0
		for x := xb; x < xe; x++ {
			for y := yb; y < ye; y++ {
				m = max(m, prod(x, xi, y, yi))
			}
		}
		return m
	}

	m := max(search(0, N, 0, 0, M-L, 1),
		max(search(0, N-L, 1, 0, M, 0),
			max(search(0, N-L, 1, 0, M-L, 1),
				search(L, N, -1, 0, M-L, 1))))
